...
- 使用确定性牌形算法计算向听、有效牌、未见枚数与听牌延伸，并与 Mortal 的
  候选动作和 Q 值分开解释。
- 结合分数、牌河、副露和可见牌解释关键决策。
- 将明确反馈和多场牌谱中的重复倾向整理为可确认、纠正或遗忘的长期画像。
- 在牌局目录中浏览和筛选本地复盘；持有个人牌谱屋 API Key 时，还可同步
  公开牌局并从中选择要复盘的对局。
...
牌局目录首先用于浏览已导入并保存在本地的复盘，支持按账号、顺位、日期和复盘
状态筛选，也可以选择一局继续看牌或复盘。使用本地目录不需要绑定雀魂账号，
也不影响直接使用雀魂牌谱链接。

如果希望额外同步牌谱屋中的公开牌局，可以绑定雀魂账号。MJTutor 使用雀魂
“个人资料”中显示的 UID 与昵称标识账号。首次关联牌谱屋时，还需要提供一条
确认属于该账号的牌谱链接。MJTutor 会从中识别牌谱屋内部账号 ID，普通使用时
无需记住或手动查找它。牌谱屋的数据可能延迟或不完整。

同步功能需要用户持有站方签发的个人 API Key，并在本机将它配置为
`MJTUTOR_KOROMO_TOKEN`。MJTutor 不运行云服务，也不会内置、共享或转发开发者的
密钥，因此公开版插件的使用者需要自行向牌谱屋申请 Key。请勿将密钥发到聊天、
Issue 或提交到 GitHub。未配置 Key 时，目录只浏览本地复盘，不会向牌谱屋发起
同步请求。
...
完整复盘时，教练会把当前决策与既往牌局作有限度的比较，主动检验可能的牌风或
弱点，也会寻找反例。只有形成新的跨局倾向、原有判断发生明显变化，或者它对当前
讲解确有帮助时才会自然提及，不会在每次回答后重复画像结论。
